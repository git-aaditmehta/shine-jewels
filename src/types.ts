...
export interface Product { id: string; designCode: string; categoryId: string; subcategoryId: string; weightMg: number; gridImage: string; detailImage: string; imageVersion: number; syncState: SyncState; deleted?: boolean }
...
export interface Order { id: string; orderNumber?: number; vendor: Vendor; salesperson: string; status: 'DRAFT' | 'FINALIZED'; createdAt: string; generatedAt?: string; items: OrderItem[] }
